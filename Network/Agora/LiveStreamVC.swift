...
class LiveStreamVC: UIViewController {
   ...
   // var agoraKit: AgoraRtcEngineKit?
    ...
    let engine = AgoraRtcEngineKit.sharedEngine(
            withAppId: "a6e3170384984816bf987ddd4aad3029",
            delegate: self
        )
...
//        agoraKit?.setClientRole(.audience, options: options)
    ...
    let  userid1  =  UserDefaults.standard.object(forKey: "userid") as? Int ?? 0
    
    func joinChannel(){
            // The uid of each user in the channel must be unique.
        self.agoraKit.enableVideo()
        agoraKit.joinChannel(byToken: "0065d5dd39d0ce547a5a9a3278663b8457aIAD8C1Jn5gpY8/p+HB2C9CpJ7hnXAUrYCyn3HZ3jHhLw+VzZdDsAAAAAEAAApPzouQX0YAEAAQC5BfRg", channelId: "bhardwaj", info: nil, uid: UInt(userid1), joinSuccess: { (channel, uid, elapsed) in
        })
    }
